...
export async function createNodeTexFiles() {
	...
	function* texFileGenerator(stencilPreBuffer, stencilMidBuffer, stencilPostBuffer, nodeList) {
		stencilPreBuffer = stencilPreBuffer instanceof Buffer ? stencilPreBuffer : Buffer.from(stencilPreBuffer, "utf8")
		stencilMidBuffer = stencilMidBuffer instanceof Buffer ? stencilMidBuffer : Buffer.from(stencilMidBuffer, "utf8")
		stencilPostBuffer =
		...
		for (const [index, node] of nodeList.entries()) {
			const optionPossibilities = optionsObjectToArray(node.options)
			for (const optionPossibility of optionPossibilities) {
				const options = optionPossibility.map((option) => option.name)
				const optionsDisplay = optionPossibility.map((option) => option.displayName ?? option.name)
				const nameOptionsList = [node.name, ...(options || [])]
				const basename = getFileBaseName(index, node.name, true, optionsDisplay)
				...
				if (node.pins && node.pins.length > 0) {
					anchorLines = node.pins
						...
						.filter((anchorName) => !subPins.includes(anchorName))
						.concat(addPins)
	...
	return new Promise((resolve) => {
		nodeTexFileStream.pipe(dest(buildFolderAbs)).on("finish", resolve)
	})
}
 ...
 * @param {any[]} options
 */
export function optionsObjectToArray(options) {
	if (!options || options.length < 1) {
		return [[]]
	}
	const currentOption = options[0]
	let convertedOptions = optionsObjectToArray(options.slice(1))

	let newConvertedOptions = []
	if (Object.hasOwn(currentOption, "enumOptions")) {
		for (const option of currentOption.enumOptions) {
			for (const convertedOption of convertedOptions) {
				let optionsArray = [option]
				optionsArray.push(...convertedOption)
				newConvertedOptions.push(optionsArray)
			}
		}
		if (currentOption.selectNone ?? true) {
			for (const convertedOption of convertedOptions) {
				newConvertedOptions.push([...convertedOption])
			}
		}
	} else {
		for (const convertedOption of convertedOptions) {
			let optionsArray = [currentOption]
			optionsArray.push(...convertedOption)
			newConvertedOptions.push(optionsArray)
			newConvertedOptions.push([...convertedOption])
		}
	}
	return newConvertedOptions
}
...
export async function createPathTexFiles() {
	...
	function* texFileGenerator(stencilPreBuffer, stencilMidBuffer, stencilPostBuffer, pathList) {
		...
		for (const [index, path] of pathList.entries()) {
			const optionPossibilities = optionsObjectToArray(path.options)
			for (const optionPossibility of optionPossibilities) {
				const options = optionPossibility.map((option) => option.name)
				const optionsDisplay = optionPossibility.map((option) => option.displayName ?? option.name)
				const shapeName = path.shapeName || path.drawName.replaceAll(" ", "") + "shape"
				const nameOptionsList = [shapeName, ...(options || [])]
				const basename = getFileBaseName(index, path.drawName, false, optionsDisplay)
				const nameBuffer = Buffer.from(nameOptionsList.join(", "), "utf8")
				...
				if (path.stroke) anchorLines += "\t\t\\draw (P.b) -- (P.a);\n"
	...
	const pathTexFileStream = Readable.from(texFileGenerator(stencilPre, stencilMid, stencilPost, NODES_AND_PATHS.path))

	return new Promise((resolve) => {
		pathTexFileStream.pipe(dest(buildFolderAbs)).on("finish", resolve)
	})
}
