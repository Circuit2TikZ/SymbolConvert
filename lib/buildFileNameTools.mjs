...
export function getFileBaseName(index, componentName, isNode = true, options = []) {
	let basenameArray = index === null ? [] : [index.toString(16).padStart(3, "0")];
	basenameArray.push(isNode ? "node" : "path", componentName);
...
export function decodeFilename(filename) {
	const regex = /^(?<index>[0-9a-f]{3})_(?<nodeOrPath>(node)|(path))_/i;
	const match = filename.match(regex);
	const index = Number.parseInt(match?.groups?.index ?? "NaN", 16);
