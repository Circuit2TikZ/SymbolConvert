...
function convertSVGtoSymbol(svgContent, fileInfo, nodeDescription, ID) {
	...
	const filteredContent = svgContent
		...
		.reduce((/** @type {string[]} */ prevVal, line) => {
			...
			try {
				...
				lines.push(line);
				console.log((nodeDescription.displayName ?? nodeDescription.name) +": Found colored line: ", line.start, line.end, line.color);
			} catch (e) {
	...
	let pinLines = lines.filter((line) => !zeroLineColors.concat([textLineColor]).find((color) => colorTools.isColorEqual(line.color, color)));
	...
	if (textLine.length>0) {
		...
		metaLines.push(`\t\t\t<ci:textPosition x="${textPoint.x}" y="${textPoint.y}"/>`)
	}
