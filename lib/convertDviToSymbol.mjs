...
export function convertDVItoSVGsymbol() {
	return vinylMap(
		...
		async function (file) {
			...
			svgContent = await convertDVItoSVG(file.path, translateX, translateY, 4 / 3, 2)
			...
			for (const optionPossibility of optionPossibilities) {
				...
				nodeDescriptionCopy.pins = nodeDescription.pins.filter((anchorName) => !subPins.includes(anchorName))
