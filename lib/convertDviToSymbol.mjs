...
import { Point, arePointsEqual, findMatchingLinePoint, svgPathToLine } from "./svgPointTools.mjs";
 ...
 */
function convertSVGtoSymbol(svgContent, basename) {
	...
	const fileInfo = decodeFilename(basename);
	...
	pinLines.forEach((line) => {
		...
		if (!arePointsEqual(line.start, refPoint)) {
			// unsorted
			if (arePointsEqual(line.end, refPoint)) {
				line.end = line.start;
				line.start = refPoint;
			} else {
	...
	const hasDefault = !pinAnchors.every((pinAnchor) => {
		if (arePointsEqual(zeroCoordinate, pinAnchor.point)) {
...
export function convertDVItoSVGsymbol() {
	return vinylMap(
		...
		async function (file) {
			...
			svgContent = convertSVGtoSymbol(svgContent, file.stem);
