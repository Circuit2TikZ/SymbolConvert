...
def _compile_worker(path:pathlib.Path):
    ...
    """
    command = ["lualatex", "-halt-on-error", "-output-format=dvi", "-interaction=nonstopmode", f'-jobname="{path.stem}"', path.absolute()]
...
def create_tex_files_from_json(output_dir="build"):
    ...
    for index,node in enumerate(NODES_AND_PATHS["nodes"]):
        ...
        for tmp,option_possibility in enumerate(option_possibilities):
            ...
            options_display = [option["displayName"] if "displayName" in option else option["name"] for option in option_possibility]
