...
import json
from utils import color_to_index, component_name_from_info, filter_newer, options_object_to_array, parse_filename
...
def _compile_DVI_to_SVG_worker(path:pathlib.Path, translateX = 0, translateY = 0, scale = 1):
    ...
    command = ["dvisvgm", "--bbox=min", "--clipjoin", "--optimize=group-attributes,remove-clippaths,simplify-transform,collapse-groups", "--no-fonts", "--stdout"]
...
def _convert_SVG_to_symbol_worker(svg_content:str, index, is_node, node_description, ID, option_possibility):
    ...
    for line in re.split(r"\r?\n", svg_content):
        ...
        try:
            ...
            l = svg_path_to_line(tag_options)
            line_color:Color = l["color"]
    ...
    text_line_colors = [Color("#f00"),Color("rgb(255,0,155)")]
    zero_line_colors = [ Color("#ff0"), Color("#0ff")]


    pin_lines = [line for line in lines if all([Color.distance_sq(line["color"],c)!=0 for c in (zero_line_colors+text_line_colors)])]
    ref_point = find_matching_line_point(lines)
    zero_coordinate = Point(0, 0)

    pin_anchors =  ["START", "END", *(node_description["pins"] if "pins" in node_description else [])] if not is_node else (node_description["pins"] if "pins" in node_description else [])
    ...
    shape_name = None if is_node else (node_description["shapeName"] if "shapeName" in node_description else node_description["drawName"].replace(" ","")+"shape")
    ...
    def format_option(option:dict)->str:
        if "enumOptions" in option:
            ...
            for enum_option in option["enumOptions"]:
                enum_lines.append(option_str_from_option(enum_option, is_option_active(enum_option["name"], option_possibility)))
    ...
    meta_lines.extend([f'\t\t\t\t<ci:pin anchorName="{pin_anchor.anchor_name}" x="{pin_anchor.point.x}" y="{pin_anchor.point.y}" {'isDefault="True" ' if pin_anchor.default else ''}/>' for pin_anchor in pin_anchors])
    meta_lines.append("\t\t\t</ci:pins>")

    text_line = [line for line in lines if Color.distance_sq(line["color"], text_line_colors[0])==0]
    if len(text_line)>0:
        text_point:Point = text_line[0]["end"]-ref_point
        meta_lines.append(
            f'\t\t\t<ci:textPosition x="${text_point.x}" y="${text_point.y}"/>'
        )
...
def find_matching_line_point(lines:list[dict]):
    ...
    for point in points[1:]:
        close = next((k for k in acc.keys() if k and point and are_two_points_close(k,point)), None)
...
def are_two_points_close(pointA:Point, pointB:Point, eps = 1e-6):
    return (pointA.x >= pointB.x - eps) and (pointA.x <= pointB.x + eps) and (pointA.y >= pointB.y - eps) and (pointA.y <= pointB.y + eps)
...
def _convert_DVI_to_symbol_worker(path:pathlib.Path):
    ...
    for option_possibility in option_possibilities:
        optionsDisplay = list(map(lambda option : option["displayName"] if "displayName" in option else option["name"], option_possibility))
        name = component_name_from_info(index, node_description["name"] if is_node else node_description["drawName"], is_node, optionsDisplay)
    ...
    if "pins" in node_description_copy:
        node_description_copy["pins"] = [pin for pin in node_description["pins"] if not pin in subPins]
    else:
        node_description_copy["pins"] = []
    node_description_copy["pins"].extend(addPins)

    ID = component_name_from_info(None, node_description["name"] if is_node else node_description["drawName"], is_node, optionsDisplay)
    ...
    p = subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, cwd=path.parent)
    ...
    if p.returncode == 0:
        returncode, returnstr, svg_content = _convert_SVG_to_symbol_worker(svg_content, index, is_node, node_description_copy, ID, option_possibility)
...
def combine_SVGs_to_symbol():
    ...
    clusteredSymbols:dict[str,list[Element]] = {}
    ...
    command = ["svgo", "--config", config_name, "-i","-", "-o", "-"]
    p = subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, cwd=".")
